 ...
 * @author chenxiaofan
 */
public abstract class AbstractClusterMultiThreadSync<T> {
    ...
    private void test(boolean autoBatchFlush, Args args) {
        ...
        try (RedisClusterClient redisClient = RedisClusterClient
                .create(RedisURI.Builder.redis(args.host).withSsl(args.useSsl).build())) {
            ...
            if (autoBatchFlush) {
                optsBuilder.autoBatchFlushOptions(AutoBatchFlushOptions.builder().enableAutoBatchFlush(true)
                        .batchSize(batchSize).writeSpinCount(AutoBatchFlushOptions.DEFAULT_WRITE_SPIN_COUNT).build());
            }
    ...
    private static class Args {
        ...
        public Args(int threadCount, int loopNum, int batchSize, String host, boolean useSsl) {
    ...
    private static Args parseArgs(String[] args) {
        ...
        return new Args(threadCount, loopNum, batchSize, host, useSsl);
    }

    protected static <T> void runTest(AbstractClusterMultiThreadSync<T> abstractClusterMultiThreadSync, String[] arguments) {
        final Args args = parseArgs(arguments);
        for (boolean autoBatchFlush : new boolean[] { false }) {
            logger.info("=====================================");
