...
public abstract class AbstractSingleThreadAsync<T> {
    ...
    private void testBatch(Common.TestParameter para, Args args, boolean printResult) {
        final int loopNum = args.loopNum;
        final int batchSize = args.batchSize;
        final String host = args.host;
    ...
    private static class Args {
        ...
        public Args(int loopNum, int batchSize, String host, int port, boolean useSsl, boolean skipWarmup) {
    ...
    private static Args parseArgs(String[] args) {
        ...
        return new Args(loopNum, batchSize, host, port, useSsl, skipWarmup);
    }
    ...
    protected static <T> void runTest(AbstractSingleThreadAsync<T> async, String[] arguments) {
        ...
        for (Common.TestParameter para : new Common.TestParameter[] { Common.TestParameter.AUTO_BATCH_WITHOUT_CONSOLIDATE_FLUSH,
                Common.TestParameter.AUTO_BATCH_WITH_CONSOLIDATE_FLUSH_ALWAYS, Common.TestParameter.NO_AUTO_BATCH }) {
            logger.info("=====================================");
            ...
            if (!args.skipWarmup) {
                ...
                async.test(para, args, false);
                logger.info("warmed up.");
            }
            async.test(para, args, true);
        }
        logger.info("=====================================");
    }
    ...
    protected static <T> void runBatchTest(AbstractSingleThreadAsync<T> async, String[] arguments) {
        ...
        for (Common.TestParameter para : new Common.TestParameter[] { Common.TestParameter.AUTO_BATCH_WITHOUT_CONSOLIDATE_FLUSH,
            ...
            if (!args.skipWarmup) {
                logger.info("warming up...");
                async.testBatch(para, args, false);
                logger.info("warmed up.");
            }
            async.testBatch(para, args, true);
        }
        logger.info("=====================================");
    }
